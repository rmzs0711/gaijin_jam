...
public:
    ...
    virtual void click_mouse_left(const sf::Vector2f &mouse,
                                  sf::Vector2f &old_mouse) = 0;
...
public:
    ...
    Object(const std::string &name_file_,
           const sf::Vector2f &position = sf::Vector2f(0, 0))
        : is_valid(true), name_file(name_file_) {
    ...
    void loadFromFile(const std::string &name_file_,
                      sf::IntRect rect = sf::IntRect(0, 0, 0, 0),
                      sf::Vector2i size_object = sf::Vector2i(0, 0)) {
...
public:
    Polygon(const std::vector<sf::Vector2f> &point,
            std::function<T()> func,
            const sf::Vector2f &position = sf::Vector2f(0, 0),
            sf::Color color = sf::Color::Transparent,
            float thickness = 10,
    ...
    virtual bool is_correct_click(const sf::Vector2f &mouse) {
        ...
        for (int i = 0; i < point.size(); i++) {
            ...
            if ((yi < mouse.y && yj >= mouse.y ||
                 yj < mouse.y && yi >= mouse.y) &&
    ...
    bool condition_for_move() {
        return sf::Mouse::isButtonPressed(sf::Mouse::Left) &&
               getOutlineColor() == sf::Color::Green;
    }
...
public:
    Circle(std::function<T()> func,
           float radius_ = 90,
           const sf::Vector2f &position = sf::Vector2f(0, 0),
           sf::Color color = sf::Color::Transparent,
           float thickness = 10,
           std::string str_ = "")
        : Polygon<T>(func, position, color, thickness, str_) {
    ...
    bool is_correct_click(const sf::Vector2f &mouse) {
        ...
        return (mouse.x - position.x - radius) *
                       (mouse.x - position.x - radius) +
                   (mouse.y - position.y - radius) *
                       (mouse.y - position.y - radius) <=
               radius * radius;
    }
    ...
    void draw(sf::RenderWindow &window) {
        ...
        CentralisedText text(Button<T>::getString());
        text.setPosition(Polygon<T>::getPosition() +
